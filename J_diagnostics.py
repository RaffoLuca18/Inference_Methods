...
def complete_experiment(J, n_samples, method, use_tol=True, isingzation = False):
    ...
    axes[1].set_yticklabels([f"{b:.3f}" for b in betas_grid])
...
def complete_experiment_noise(J, n_samples, method, p = 0.1, use_tol=True):
    """ full experiment, as beta and n_samples vary, and the samples are noisy with parameter p """
    ...
    for i, beta in enumerate(betas_grid):
        ...
        for j, n_samples in enumerate(samples_grid):
            ...
            samples_n = J_sampler.J_sampler_noise(n_samples, J, h, p)
            histogram_n = J_sampler._samples_to_histogram(samples_n)
